 ...
 */
@SourceType(value = "SHENTONG", label = "ShenTong")
public class ShenTongConf extends AbstractArpConf<ShenTongConf> {

    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(ShenTongConf.class);

    private static final String ARP_FILENAME = "arp/implementation/shentong-arp.yaml";

    private static final ArpDialect ARP_DIALECT = AbstractArpConf.loadArpFile(ARP_FILENAME, (ArpDialect::new));
    private static final String DRIVER = "com.oscar.Driver";
    ...
    public String toJdbcConnectionString() {
        ...
        final String serviceName = checkNotNull(this.serviceName, "Missing database.");

        logger.info(String.format("JDBC URL ---- jdbc:oscar://%s:%s/%s", host, port, serviceName));
        return String.format("jdbc:oscar://%s:%s/%s", host, port, serviceName);
    }
